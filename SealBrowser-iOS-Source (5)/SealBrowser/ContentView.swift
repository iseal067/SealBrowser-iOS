        ...
        HStack(spacing: 6) {
            ...
            if addressFocused {
                TextField("Search or enter website", text: $addressText)
                    ...
                    .tint(SealUI.accent)
                    .onSubmit {
                        tab.open(addressText)
                        addressFocused = false
                    }
            } else {
                Text(tab.address.isEmpty ? "Search or enter website" : tab.displayHost)
                    .font(.system(size: 14.5, weight: tab.address.isEmpty ? .regular : .medium))
                    .foregroundStyle(tab.address.isEmpty ? SealUI.muted : SealUI.text)
                    .lineLimit(1)
                    .frame(maxWidth: .infinity, alignment: tab.address.isEmpty ? .leading : .center)
            }
